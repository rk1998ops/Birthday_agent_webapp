...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .wrap{position:relative;z-index:1;max-width:980px;margin:0 auto;padding:48px 24px 96px}
    ...
    /* Tabs — now control BOTH the send mode AND the list view */
    ...
    .ct-meta{font-size:.8rem;color:var(--muted);margin-top:2px}
    ...
    .srch input{background:none;border:none;outline:none;color:var(--text);font-family:'DM Sans',sans-serif;font-size:.88rem;width:160px}
    ...
    .bday-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:16px;padding:24px 28px}
    .bday-card{background:rgba(245,200,66,.06);border:1px solid rgba(245,200,66,.2);border-radius:16px;padding:20px;display:flex;align-items:center;gap:16px;transition:transform .2s,border-color .2s;animation:up .4s ease both}
    .bday-card:hover{transform:translateY(-3px);border-color:rgba(245,200,66,.45)}
    .bday-avatar{width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,var(--gold),var(--rose));display:flex;align-items:center;justify-content:center;font-size:1.4rem;flex-shrink:0}
    .bday-info{flex:1;min-width:0}
    .bday-name{font-weight:500;font-size:.95rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .bday-contact{font-size:.78rem;color:var(--muted);margin-top:2px}
    .bday-status{margin-top:8px}
    ...
    .no-bday{padding:48px 28px;text-align:center}
    .no-bday-icon{font-size:3.5rem;margin-bottom:16px;opacity:.6}
    .no-bday p{color:var(--muted);font-size:.95rem;line-height:1.6}
    .no-bday strong{color:var(--text)}
    ...
    .t-ok .tp{background:var(--ok)}.t-err .tp{background:var(--err)}.t-inf .tp{background:var(--gold)}
    .tclose{background:none;border:none;color:var(--muted);cursor:pointer;font-size:1rem;padding:0;flex-shrink:0;transition:color .2s}
    ...
    @media(max-width:600px){
      .stats{gap:8px}.sv{font-size:1.5rem}.card{padding:22px}
      .bday-grid{grid-template-columns:1fr;padding:16px}
      thead{display:none}
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <!-- Tabs control both the list view AND send mode -->
    <div class="tabs">
      <button class="tab active" id="tab-today" onclick="switchTab('today')">🎂 Today's Birthdays</button>
      <button class="tab"        id="tab-all"   onclick="switchTab('all')">👥 All Contacts</button>
    </div>
  ...
  <!-- Contact list panel — switches between two views -->
  <div class="ct">
    <div class="ct-hdr">
      <div>
        <h2 class="ct-title" id="list-title">Today's Birthdays</h2>
        <div class="ct-meta" id="list-meta"></div>
      </div>
      <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
        <!-- Search only shown in All Contacts view -->
        <div class="srch" id="srch-box" style="display:none">
          ...
          <input id="q" type="text" placeholder="Search contacts…" oninput="filterAll()"/>
        </div>
    ...
    <!-- Today's birthdays view (cards) -->
    <div id="view-today"><div class="empty"><div class="ei">📋</div><p>Loading…</p></div></div>

    <!-- All contacts view (table) — hidden by default -->
    <div id="view-all" style="display:none"><div class="empty"><div class="ei">📋</div><p>Loading…</p></div></div>
  </div>
...
/* ── Switch tab — controls both the list view and the send mode ── */
function switchTab(m){
  mode=m;
  document.getElementById('tab-today').classList.toggle('active', m==='today');
  document.getElementById('tab-all').classList.toggle('active',   m==='all');
  document.getElementById('view-today').style.display = m==='today' ? '' : 'none';
  document.getElementById('view-all').style.display   = m==='all'   ? '' : 'none';
  document.getElementById('srch-box').style.display   = m==='all'   ? '' : 'none';
  document.getElementById('sbtxt').textContent =
    m==='today' ? '🎉 Send Birthday Wishes' : '🌟 Send Wishes to All Contacts';
  document.getElementById('list-title').textContent =
    m==='today' ? "Today's Birthdays" : 'All Contacts';
  updateMeta();
}

function updateMeta(){
  const today = contacts.filter(c=>c.is_birthday_today);
  document.getElementById('list-meta').textContent =
    mode==='today'
      ? today.length + ' contact' + (today.length!==1?'s':'') + ' celebrating today'
...
async function load(){
  ...
  try{
    ...
    renderAll(contacts);
    updateMeta();
    toast('inf','📋 Loaded',d.total+' contacts · '+d.birthdays_today+' birthday'+(d.birthdays_today!==1?'s':'')+' today');
  }catch(e){
    document.getElementById('view-today').innerHTML=err(e.message);
    document.getElementById('view-all').innerHTML=err(e.message);
...
function setLoadingState(){
  const loading='<div class="empty"><div class="ei">⚙️</div><p>Fetching from Google Sheets…</p></div>';
  document.getElementById('view-today').innerHTML=loading;
  document.getElementById('view-all').innerHTML=loading;
}

/* ── Render Today's Birthdays as cards ── */
function renderToday(){
  const today=contacts.filter(c=>c.is_birthday_today);
  if(!today.length){
    document.getElementById('view-today').innerHTML=
      '<div class="no-bday"><div class="no-bday-icon">🎈</div>'+
      '<p><strong>No birthdays today!</strong><br>Switch to <em>All Contacts</em> to view and message everyone.</p></div>';
    return;
  }
  const cards=today.map((c,i)=>{
    let badge='';
    if(c.already_sent)      badge='<span class="badge b-sent">✓ Wish Sent</span>';
    else if(!c.has_telegram) badge='<span class="badge b-notg">No Telegram ID</span>';
    else                     badge='<span class="badge b-bday">🎂 Ready to Wish</span>';
    const initials=c.name.split(' ').map(w=>w[0]||'').join('').slice(0,2).toUpperCase();
    return '<div class="bday-card" style="animation-delay:'+i*.05+'s">'+
      '<div class="bday-avatar">'+initials+'</div>'+
      '<div class="bday-info">'+
        '<div class="bday-name">'+x(c.name)+'</div>'+
        '<div class="bday-contact">'+x(c.contact_number)+'</div>'+
        '<div class="bday-status" style="margin-top:8px">'+badge+'</div>'+
      '</div></div>';
  }).join('');
  document.getElementById('view-today').innerHTML='<div class="bday-grid">'+cards+'</div>';
}

/* ── Render All Contacts as table ── */
function renderAll(data){
  if(!data.length){
    document.getElementById('view-all').innerHTML='<div class="empty"><div class="ei">🔍</div><p>No contacts found.</p></div>';
    return;
  }
  const rows=data.map(c=>{
    ...
    return '<tr>'+
      '<td style="font-weight:500">'+x(c.name)+'</td>'+
      '<td style="color:var(--muted)">'+x(c.contact_number)+'</td>'+
      '<td>'+dob+'</td>'+
      '<td>'+(c.telegram_chat_id?'<code style="font-size:.78rem;color:var(--teal)">'+c.telegram_chat_id+'</code>':'<span style="color:var(--muted)">—</span>')+'</td>'+
      '<td>'+b+'</td></tr>';
  }).join('');
  document.getElementById('view-all').innerHTML=
    '<table><thead><tr><th>Name</th><th>Contact</th><th>Date of Birth</th><th>Telegram Chat ID</th><th>Status</th></tr></thead><tbody>'+rows+'</tbody></table>';
}

/* ── Search (All Contacts tab only) ── */
function filterAll(){
  const q=document.getElementById('q').value.toLowerCase();
  renderAll(contacts.filter(c=>c.name.toLowerCase().includes(q)||c.contact_number.toLowerCase().includes(q)));
}
...
async function sendWishes(){
  ...
  try{
    ...
    const items=(d.results||[]).map(r=>{
      ...
      return '<div class="ri"><div class="dot '+cls+'"></div><span>'+x(r.name)+'</span>'+
        '<span style="color:var(--muted);font-size:.78rem;text-transform:capitalize">'+r.status+'</span>'+rsn+'</div>';
    }).join('');
    ...
    if(d.sent>0){
      toast('ok','🎉 '+d.sent+' wish'+(d.sent>1?'es':'')+' sent!',d.message);
      confetti(); load();
    } else if(d.skipped>0&&d.sent===0){
      toast('inf','Nothing new',d.message||'All wishes already sent or no birthdays today.');
    } else {
      toast('err','Nothing sent',d.message||'Check configuration.');
    }
  }catch(e){toast('err','Error',e.message);}
  finally{
    btn.disabled=false; sp.classList.remove('on');
    txt.textContent=mode==='today'?'🎉 Send Birthday Wishes':'🌟 Send Wishes to All Contacts';
  }
}
...
function toast(type,title,msg,dur=4200){
  ...
  t.innerHTML='<div class="ti">'+icons[type]+'</div><div class="tb"><div class="tt">'+title+'</div><div class="tm">'+msg+'</div></div>'+
    '<button class="tclose" onclick="rmToast(this.parentElement)">✕</button><div class="tp"></div>';
...
function confetti(){
  ...
  const ps=Array.from({length:130},()=>({
    ...
    r:Math.random()*6+3,d:Math.random()*60+30,
    c:cols[~~(Math.random()*cols.length)],ta:0,tai:Math.random()*.07+.05,
    s:Math.random()>.5?'r':'c'
  }));
  let a=0,fr;
  function draw(){
...
function err(msg){return '<div class="empty"><div class="ei">⚠️</div><p style="color:var(--err)">'+msg+'</p></div>';}
